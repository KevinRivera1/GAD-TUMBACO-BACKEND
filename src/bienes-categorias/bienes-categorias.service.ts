...
export class BienesCategoriasService {
  ...
  async create(createBienesCategoriaDto: CreateBienesCategoriaDto) {
    ...
    const existingInMemory = this.categorias.find(
      (categoria) =>
        ...
        createBienesCategoriaDto.nombre_categoria,
    );

    if (existingCategoria || existingInMemory) {
      throw new ConflictException(
        `La categoría ${createBienesCategoriaDto.nombre_categoria} ya existe`,
      );
    }
  ...
  async findOne(id: number): Promise<BienesCategoria> {
    const categorias = await this.categoriasRepository.findOneBy({
      id_categorias: id,
    });
    if (!categorias)
  ...
  async update(id: number, updateBienesCategoriaDto: UpdateBienesCategoriaDto) {
    ...
    if (!categoria)
      throw new NotFoundException(
        'No se pudo encontrar la categoria con el ID proporcionado',
      );
    ...
    const existingInMemory = this.categorias.find(
      (categoria) =>
        categoria.nombre_categoria ===
        updateBienesCategoriaDto.nombre_categoria,
    );

    if (existingCategoria || existingInMemory) {
      throw new ConflictException(
        `La categoría ${updateBienesCategoriaDto.nombre_categoria} ya existe`,
      );
    }
